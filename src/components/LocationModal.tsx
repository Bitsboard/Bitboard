...
export function LocationModal({ open, onClose, initialCenter, initialRadiusKm = 25, onApply, dark }: LocationModalProps) {
	...
	React.useEffect(() => {
		...
		(async () => {
			...
			const marker = (L as any).marker([center.lat, center.lng], { draggable: true }).addTo(map);
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
						...
						<div>
							...
							<select
								...
								className={cn("w-40 sm:w-48 rounded-xl px-4 py-3 text-sm border appearance-none bg-no-repeat pr-10", dark ? "border-neutral-700 bg-neutral-800 text-neutral-100" : "border-neutral-300 bg-white text-neutral-900")}
