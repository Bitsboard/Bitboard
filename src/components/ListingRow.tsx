...
export function ListingRow({ listing, unit, btcCad, dark, onOpen }: ListingRowProps) {
  ...
  return (
    <article
      onClick={onOpen}
      className={cn(
        "group relative grid grid-cols-12 gap-4 overflow-hidden rounded-2xl p-4 transition-all duration-300 hover:scale-[1.03] hover:ring-2 hover:ring-orange-400/70 hover:shadow-[0_25px_70px_-12px_rgba(255,140,0,0.45)] cursor-pointer min-h-[10rem] sm:min-h-[11rem] md:min-h-[12rem]",
    ...
    >
      ...
      <div className="col-span-3 overflow-hidden rounded-l-2xl -ml-4 -my-4 mr-0 h-[10rem] sm:h-[11rem] md:h-[12rem]">
        <div className="relative h-full">
          <Carousel images={[listing.images[0]]} alt={listing.title} dark={dark} className="h-full" rounded="rounded-l-2xl" showDots={false} showArrows={false} />
