...
export async function GET(req: Request) {
  try {
    ...
    const res = await db.prepare('SELECT id, email, username, verified, is_admin AS isAdmin, banned, created_at AS createdAt FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?').bind(limit, offset).all();
    const count = await db.prepare('SELECT COUNT(*) AS c FROM users').all();
