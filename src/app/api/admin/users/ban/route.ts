...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as { userId?: unknown; banned?: unknown };
    const userId = (body.userId ?? '').toString();
    const banned = String(body.banned) === 'true' || body.banned === true ? 1 : 0;
    if (!userId) return new Response(JSON.stringify({ error: 'invalid_user' }), { status: 400 });
    await db.prepare('UPDATE users SET banned = ? WHERE id = ?').bind(banned, userId).run();
