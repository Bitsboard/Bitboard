...
export async function GET(req: Request) {
  try {
    ...
    const res = await db.prepare(`SELECT id, title, price_sat AS priceSat, posted_by AS postedBy, created_at AS createdAt FROM listings ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(...binds, limit, offset).all();
    const count = await db.prepare(`SELECT COUNT(*) AS c FROM listings ${where}`).bind(...binds).all();
