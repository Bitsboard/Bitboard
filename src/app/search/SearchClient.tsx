...
import { ListingCard, ListingRow, Nav, ViewToggle, ListingModal } from "@/components";
...
export default function SearchClient() {
  ...
  }, [q, selCategory, selAdType, minPrice, maxPrice, sortChoice, unit, country, region, city, centerLat, centerLng]);
  ...
  const regions = useMemo(() => {
    ...
    const regionsByCountry = LOCATIONS.filter(l => l.country === country);
    const regions = Array.from(new Set(regionsByCountry.map(l => l.region)));
    const cities = regionsByCountry.filter(l => l.region === region).map(l => l.city);
          ...
          <aside className="lg:col-span-3 space-y-4">
            ...
            <div className={cn("rounded-2xl p-4", dark ? "border border-neutral-800 bg-neutral-950" : "border border-neutral-300 bg-white")}> 
              ...
              <div className="flex flex-col gap-2">
                <select value={country} onChange={(e) => { setCountry(e.target.value); setRegion(''); setCity(''); }} className={cn("rounded-xl px-3 py-2 text-sm", inputBase)}>
                  <option value="">Country</option>
                  {Array.from(new Set(LOCATIONS.map(l => l.country))).map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <select value={region} onChange={(e) => { setRegion(e.target.value); setCity(''); }} className={cn("rounded-xl px-3 py-2 text-sm", inputBase)}>
                  <option value="">Region</option>
                  {Array.from(new Set(LOCATIONS.filter(l => l.country === country).map(l => l.region))).map(r => <option key={r} value={r}>{r}</option>)}
                </select>
                <select value={city} onChange={(e) => setCity(e.target.value)} className={cn("rounded-xl px-3 py-2 text-sm", inputBase)}>
                  <option value="">City</option>
                  {LOCATIONS.filter(l => l.country === country && l.region === region).map(c => <option key={c.city} value={c.city}>{c.city}</option>)}
                </select>
                <p className={cn("mt-1 text-xs", dark ? "text-neutral-400" : "text-neutral-600")}>Used when sorting by Closest.</p>
              </div>
            </div>
