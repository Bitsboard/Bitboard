...
function generateSQL() {
  let sql = '-- Generate 100 more listings (101-200)\n\n';
  ...
  for (let i = 101; i <= 200; i++) {
    ...
    listings.push(`(
      ...
      ${priceSat},
      '${pricingType}',
      (SELECT id FROM user_ids LIMIT 1 OFFSET ${(i - 101) % 100}),
      ${createdAt},
      ${createdAt},
      'active',
      0,
  ...
  const imageInserts = [];
  for (let i = 101; i <= 200; i++) {
    const listingId = `(SELECT id FROM listings ORDER BY created_at DESC LIMIT 1 OFFSET ${200 - i})`;
...
fs.writeFileSync('d1/seed_listings_101_200.sql', sql);
console.log('Generated SQL for listings 101-200');
